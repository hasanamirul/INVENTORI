...
@section('content')
    ...
    <form action="{{ route('admin.barang-masuk.update', $barangMasuk->id) }}" method="POST">
        ...
        <select name="barang_id" class="border p-2 w-full mb-4" required>
            @foreach ($barang as $b)
                <option value="{{ $b->id }}" @if ($barangMasuk->barang_id == $b->id) selected @endif>
                    {{ $b->nama }}
                </option>
            @endforeach
        </select>
        ...
        <input type="number" name="jumlah" value="{{ $barangMasuk->jumlah }}" class="border p-2 w-full mb-4" required>

        <label class="block mb-2">Tanggal</label>
        <input type="date" name="tanggal" value="{{ $barangMasuk->tanggal_masuk }}" class="border p-2 w-full mb-4" required>
